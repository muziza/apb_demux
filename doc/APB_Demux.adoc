...
=== Назначение блока
APB демультиплексор - дает доступ одному APB master устройству ко многим APB slave устройствам. Каждому slave устройству параметрами назначается адресный диапазон. Выбор происходит при попадании `paddr_i` в соответствующий диапазон
...
| BASE_ADDR       | APB_ADDR_WIDTH    | Массив глубиной APB_SLAVE_COUNT. Default: 0
| ADDR_OFFSET     | APB_ADDR_WIDTH    | Сдвиг относительно базового адреса соответствующего slave. Массив глубиной APB_SLAVE_COUNT. Default: 0
|===
...
* На основе входного адреса `paddr` формируется сигнал `in_range[n-1:0]`, где каждый бит указывает на попадание адреса в соответствующий диапазон
* Выполняется проверка корректности декодирования адреса:
** Если количество единичных битов в сигнале `in_range` отличается от 1, активируется бит ошибки `en[n]`
** При активном сигнале `en[n]` на выходном интерфейсе APB Master выставляются сигналы `data_n`, `ready_n` и `slverr_n`, определенные внутри модуля
...
** Сигнал `en[n-1:0]` управляет мультиплексором Nx1 для передачи данных от slave устройства
